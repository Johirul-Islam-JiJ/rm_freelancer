...
class BuyerController extends Controller
{
    public function home()
    {
        ...
        $totalBookedService     = Booking::where('service_id', '!=', 0)->where('buyer_id', $authId)->count();
        $totalPurchasedSoftware = Booking::where('software_id', '!=', 0)->where('buyer_id', $authId)->count();
    ...
    public function bookedServiceDetails($orderNumber)
    {
        ...
        $details       = Booking::checkService($orderNumber)->where('buyer_id', auth()->id())->firstOrFail();
    ...
    {
        $booking =  Booking::checkService($orderNumber)->where('buyer_id', auth()->id())->where('status', Status::BOOKING_APPROVED)->where(function ($q) {
    ...
    public function softwarePurchase()
    {
        ...
        $softwareLog = Booking::where('software_id', '!=', 0)->where('buyer_id', auth()->id())->with('seller')->latest()->paginate(getPaginate());
