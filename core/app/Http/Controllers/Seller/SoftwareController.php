...
class SoftwareController extends Controller
{
    ...
    public function new()
    {
        ...
        $categories = Category::active()->orderBy('name')->with(['subcategories'=>function($subcategory){
            $subcategory->active();
        }])->get();
    ...
    public function salesLog()
    {
        ...
        $softwareLog = Booking::where('software_id', '!=', 0)->where('seller_id', auth()->id())->with('buyer')->latest()->paginate(getPaginate());
    ...
    protected function checkData($request)
    {
        ...
        if ($request->features) {
            $features = $features->find($request->features);
