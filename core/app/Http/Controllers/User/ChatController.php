...
class ChatController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($request->type == 'service') {
            $data = Booking::where('id', decrypt($request->id))->where('service_id', '!=', 0)->where(function($checkUser) use ($user) {
            ...
            $data = JobBid::where('id', decrypt($request->id))->where(function ($checkUser) use ($user) {
