...
class UserController extends Controller
{
    ...
    public function userDataSubmit(Request $request)
    {
        ...
        if (auth()->user()->user_type == 1) {
            ...
            return to_route('user.seller.home')->withNotify($notify);
        } elseif (auth()->user()->user_type == 2) {

            $notify[] = ['success', 'Registration process completed successfully'];
            return to_route('user.buyer.home')->withNotify($notify);
    ...
    public function workFileUpload(Request $request, $orderNumberOrJobId)
    {
        ...
        if ($request->work_type == 'service') {
            $data = Booking::checkService($orderNumberOrJobId)->where('status', Status::APPROVED)->where(function ($user) use ($authId) {
    ...
    public function dispute(Request $request, $orderNumberOrBidId)
    {
        ...
        if ($request->dispute_type == 'service') {
            $data = Booking::checkService($orderNumberOrBidId)->where(function ($checkUser) use ($user) {
