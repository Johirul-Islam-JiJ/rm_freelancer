            ...
            <div class="card custom--card">
                ...
                <div class="card-body row  pt-3">
                        ...
                        <select class="form-control" name="category_id" id="category" required>
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @selected($category->id == old('category_id'))
                                    data-subcategories="{{ @$category->subcategories }}">{{ __($category->name) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group col-lg-6">
                        ...
                        <div class="d-flex gap-3 flex-wrap">
                            @foreach ($features as $feature)
                                <div class="form-group custom-check-group">
                                    <input type="checkbox" @checked(in_array($feature->id, old('features') ?? [])) name="features[]"
                    ...
                    <div class="form-group col-xl-12 col-lg-12 form-group">
                        ...
                        <textarea class="form-control bg--gray" id="description" name="description">{{ old('description') }}</textarea>
                    </div>
                    <div class="form-group col-xl-12 col-lg-12 form-group">
                        <label class="required">@lang('Requirement')</label>
                        <textarea class="form-control bg--gray" id="requirement" name="requirement">{{ old('requirement') }}</textarea>
                    </div>
                            ...
                            <div class="thumb">
                                <div class="avatar-preview">
                                    <div class="profilePicPreview bg_img"
                                        data-background="{{ getImage('', getFileSize('service')) }}">
