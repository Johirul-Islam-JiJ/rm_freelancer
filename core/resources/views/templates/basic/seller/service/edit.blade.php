...
@section('content')
    <form class="row gy-4" action="{{ route('user.seller.service.store', $service->id) }}" method="POST"
        ...
        <div class="col-12">
            <div class="d-flex flex-wrap align-items-center justify-content-between">
                ...
                <a href="{{ route('user.seller.service.index') }}" class="btn btn--base"><i class="las la-list"></i>
                    @lang('All Service')</a>
            </div>
        </div>
        <div class="col-12">
            <div class="card custom--card">
                ...
                <div class="card-body row pt-2">
                    <div class="col-12 form-group">
                        ...
                        <input type="text" name="name" maxlength="255" value="{{ $service->name }}"
                    ...
                    <div class="col-lg-6 form-group">
                        ...
                        <select class="form-control bg--gray" name="category_id" id="category" required>
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" data-subcategories="{{ @$category->subcategories }}">
                    ...
                    <div class="col-lg-6 form-group">
                        ...
                        <div class="input-group">
                            <input type="number" step="any" min="0" class="form-control" name="price"
                                value="{{ getAmount($service->price) }}" placeholder="@lang('Enter Price')" required>
                    ...
                    <div class="col-lg-6 form-group">
                        ...
                        <div class="input-group">
                            <input type="number" min="1" class="form-control" name="max_order_qty"
                                value="{{ $service->max_order_qty }}" placeholder="@lang('Enter Quantity')" required>
                    ...
                    <div class="col-lg-6 form-group">
                        ...
                        <div class="input-group">
                            <input type="number" class="form-control" name="delivery_time"
                                value="{{ $service->delivery_time }}" placeholder="@lang('Delivery Time')" required>
                        ...
                        <div class="d-flex gap-3 flex-wrap">
                            @foreach ($features as $feature)
                                <div class="form-group custom-check-group">
                                    <input type="checkbox" name="features[]" id="features_{{ $feature->id }}"
                    ...
                    <div class="col-12 form-group">
                        ...
                        <textarea class="form-control bg--gray nicEdit" name="requirement">{{ $service->requirement }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div class="thumb">
                                <div class="avatar-preview">
                                    <div class="profilePicPreview bg_img"
                                        data-background="{{ getImage(getFilePath('service') . '/' . $service->image, getFileSize('service')) }}">
                                ...
                                <div class="avatar-edit">
                                    ...
                                    <small>@lang('Supported files'): @lang('jpeg'), @lang('jpg'), @lang('png').
                                        @lang('Image will be resized into ') <b>{{ getFileSize('service') }}</b> @lang('px')</small>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="row">
                                    @foreach ($service->extra_image as $extraImage)
                                        <div class="col-xl-4 col-lg-4 col-sm-4 col-4 form-group">
                                            <span class="remove-image-two confirmationBtn" type="button"
                                                data-question="@lang('Are you sure to remove this image?')"
                                                data-action="{{ route('user.image.remove', [$service->id, $extraImage, 'service']) }}">
                                                <i class="las la-times"></i>
                                            </span>
                                            <img class="optional_img"
                                                src="{{ getImage(getFilePath('extraImage') . '/' . $extraImage, getFileSize('extraImage')) }}">
                                        </div>
                                    @endforeach
                                </div>
                            @endif
        ...
        <div class="col-12">
            <div class="card custom--card">
                <div class="card-header d-flex flex-wrap align-items-center justify-content-between">
                    ...
                    <div class="card-btn">
                        <button type="button" class="btn btn--base btn--sm addExtra"><i
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="form-group">
                                                <input type="text" class="form-control"
                                                    name="extra_service[{{ $loop->index }}][name]"
                                                    value="{{ $extraService->name }}">
                                                <input type="hidden" name="extra_service[{{ $loop->index }}][id]"
                                        ...
                                        <div class="col-lg-4">
                                            <div class="input-group mb-3">
                                                <input type="text" class="form-control"
                                                    name="extra_service[{{ $loop->index }}][price]"
                                                    value="{{ getAmount($extraService->price) }}">
                                                <span class="input-group-text">{{ __($general->cur_text) }}</span>
                                            </div>
                                        </div>
                                        <div class="col-lg-2">
                                            @if ($extraService->status == Status::DISABLE)
                                                <button type="button" class="btn btn--success h-45 confirmationBtn w-100"
                                                    data-question="@lang('Are you sure to enable this')?"
                                                    data-action="{{ route('user.seller.service.extra.service.status', [$service->id, $extraService->id]) }}">
                                                    @lang('Enable')
                                                </button>
                                            @else
                                                <button type="button" class="btn btn--danger h-45 confirmationBtn w-100"
        ...
        $('#category').on('change', function() {
            ...
            if (subcategories && subcategories.length > 0) {
                $.each(subcategories, function(i, v) {
                    html +=
                        `<option value="${v.id}" ${v.id == `{{ $service->sub_category_id }}` ? 'selected': '' }>@lang('${v.name}')</option>`;
                });
            }
