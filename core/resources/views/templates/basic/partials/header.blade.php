                    ...
                    <nav class="navbar navbar-expand-lg p-0">
                        ...
                        <div class="collapse navbar-collapse" id="navbarSupportedContent">
                            <ul class="navbar-nav main-menu ms-auto me-auto">
                                ...
                                <li><a class="{{ menuActive('service') }}" href="{{ route('service') }}">@lang('SERVICE')</a></li>
                                <li><a class="{{ menuActive('software') }}" href="{{ route('software') }}">@lang('SOFTWARE')</a></li>
                                <li><a class="{{ menuActive('job') }}" href="{{ route('job') }}">@lang('JOB')</a></li>

                                @if ((request()->routeIs('user*') && auth()->check()) || (request()->routeIs('ticket*') && auth()->check()))
                                    <li><a class="{{ menuActive('user.seller*') }}" href="{{ route('user.seller.home') }}">@lang('SELLER')</a></li>
                                    <li><a class="{{ menuActive('user.buyer*') }}" href="{{ route('user.buyer.home') }}">@lang('BUYER')</a></li>
                                    <li><a class="{{ menuActive('user.inbox*') }}" href="{{ route('user.inbox.list') }}">@lang('INBOX')</a></li>
                                    <li><a class="{{ menuActive('ticket*') }}" href="{{ route('ticket.index') }}">@lang('SUPPORT')</a></li>
                                @else
                            ...
                            @if ((request()->routeIs('user*') && auth()->check()) || (request()->routeIs('ticket*') && auth()->check()))
                                <div class="header-right dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu--sm p-0 border-0 dropdown-menu-right">
                                        <a class="dropdown-menu__item d-flex align-items-center px-3 py-2" href="{{ route('user.seller.home') }}">
        ...
        <div class="header-short-menu">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <ul class="short-menu">
                            <li class="short-menu-close-btn-area"> <button class="short-menu-close-btn" type="button">@lang('Close')</button></li>
                            @foreach ($categories as $category)
                                <li><a href="{{ route('category.wise.product', [slug($category->name), $category->id]) }}">{{ __($category->name) }}</a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
